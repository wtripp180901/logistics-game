	...
	public static Feature build(FEATURES featureType,bool temporary,Tile parent)
    {
        switch (featureType)
        {
            ...
            case FEATURES.AIRPORT:
                return new TransportHubFeature(featureType, temporary, parent, new FEATURES[2] { FEATURES.RAIL, FEATURES.ROAD },makeStorage(featureType));
            case FEATURES.FOOD_FACTORY:
            case FEATURES.FARM:
            case FEATURES.MARKET:
                return new TransportHubFeature(featureType, temporary, parent, new FEATURES[1] { FEATURES.ROAD }, makeStorage(featureType));
